...
NATIVELB_PROVIDER=${NATIVELB_PROVIDER:-${PROVIDER}}

source ${NATIVELB_PATH}hack/config-default.sh

# Allow different providers to override default config values
test -f "hack/config-provider-${NATIVELB_PROVIDER}.sh" && source hack/config-provider-${NATIVELB_PROVIDER}.sh
