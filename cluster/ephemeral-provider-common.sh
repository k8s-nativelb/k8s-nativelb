...
_cli="docker run --privileged --net=host --rm ${USE_TTY} -v /var/run/docker.sock:/var/run/docker.sock quay.io/k8s-nativelb/gocli"
...
function prepare_config() {
    BASE_PATH=${NATIVELB_PATH:-$PWD}
    cat >hack/config-provider-$NATIVELB_PROVIDER.sh <<EOF
master_ip=$(_main_ip)
docker_tag=devel
kubeconfig=${BASE_PATH}/cluster/$NATIVELB_PROVIDER/.kubeconfig
kubectl=${BASE_PATH}/cluster/$NATIVELB_PROVIDER/.kubectl
docker_prefix=localhost:$(_port registry)/k8s-nativelb
manifest_docker_prefix=registry:5000/k8s-nativelb
EOF
}
...
function _add_common_params() {
    local params="--nodes ${NATIVELB_NUM_NODES} --memory 4096M --cpu 4 --random-ports --background --prefix $provider_prefix --registry-volume $(_registry_volume) kubevirtci/${image} ${NATIVELB_PROVIDER_EXTRA_ARGS}"
...
function _kubectl() {
    ...
    ${NATIVELB_PATH}cluster/$NATIVELB_PROVIDER/.kubectl "$@"
}
